	...
	<body>
				...
				<div class="container">
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<a href="<?= base_url(); ?>us_usuarios/agregar_usuario">
							<button type="button" class="btn btn-light-grey">
								<i class="clip-user-plus"></i> Usuario
							</button>
							</a>
							<?php } ?>
						</div>
							...
							<div class="table-responsive">
								<table class="table table-bordered table-hover" id="sample-table-1">
									...
									<?php foreach ($usuarios as $row): ?>
										<tr>
											<td><?= $row->us_id_usuario; ?></td>
											<td><?= $row->us_usuario; ?></td>
											<td><?= $row->us_nombre.' '.$row->us_apellido_paterno; ?></td>
											<td><?= $row->us_correo; ?></td>
											<td><?= $row->nu_nombre; ?></td>
											<td><?= $row->us_status==1?"Activo":"Inactivo";?></td>
											<td>
												<?php if($this->session->userdata('us_nivel_usuario') == 'Administrador') {
													if($status == 1){
												 ?>
												<a href="<?= base_url(); ?>us_usuarios/editar_usuario/<?= $row->us_id_usuario ?>">
												<button type="button" class="btn btn-xs btn-teal tooltips" data-placement="top" data-original-title="Editar Usuario">
													<i class="fa fa-edit"></i>
												</button>
												</a>
												<button type="button" class="btn btn-xs btn-red tooltips status_usuario" attrid="<?= $row->us_id_usuario; ?>" attrstatus="<?= $row->us_status; ?>" data-placement="top" data-original-title="Eliminar Usuario">
													<i class="fa fa-trash"></i>
												</button>
												<?php }else{ ?>
												<button type="button" class="btn btn-xs btn-primary tooltips status_usuario" attrid="<?= $row->us_id_usuario; ?>" attrstatus="<?= $row->us_status; ?>" data-placement="top" data-original-title="Reactivar Usuario">
		...
		<script>
			$('.check_filtro_1').on('ifChecked', function (){
			    ...
			    location.href='<?php echo base_url();?>us_usuarios/us_listado/'+valor;
		    });
			...
			$('.status_usuario').click(function(){
				var us_id_usuario = $(this).attr('attrid');
				var estado	 	  = $(this).attr('attrstatus');
				var us_status, mensaje;
				if(estado == 1){
					us_status = 0;
					...
					buttons: {
						formSubmit: {
						    ...
						    action: function () {
						    	$.ajax({
						    		type: 'POST',
						    		url: '<?= base_url(); ?>us_usuarios/cambiar_status_usuario',
						    		data: 'us_id_usuario='+us_id_usuario+'&us_status='+us_status,
						    		dataType: 'json',
